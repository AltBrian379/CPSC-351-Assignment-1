...
void* protect_critical_region(void* n) {
  ...
  while (!finished) {
				 sem_init(&mutex, 0, 1);sem_init(&mutex2, 0, 1);sem_init(&mutex3, 0, 1);// TODO: FILL IN THIS LINE		
    for (int j = 0; j < thread_num; ++j) { printf("\t\t\t"); }
    printf("CRITICAL(T%d)\n", thread_num);
				sem_wait(&mutex);sem_wait(&mutex2);sem_wait(&mutex3);// TODO: FILL IN THIS LINE

    sleep(2);

				sem_post(&mutex);sem_post(&mutex2);sem_post(&mutex3);// TODO: FILL IN THIS LINE
    for (int j = 0; j < thread_num; ++j) { printf("\t\t\t"); }
    ...
    printf("into non-critical(T%d)\n", thread_num);
				sem_destroy(&mutex); sem_destroy(&mutex2); sem_destroy(&mutex3); // TODO: FILL IN THIS LINE
  }
  return NULL;
}
